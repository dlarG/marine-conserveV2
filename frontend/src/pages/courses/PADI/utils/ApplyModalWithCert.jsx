...
const ApplyModalWithCert = ({ isOpen, onClose, courseTitle, apiBaseUrl }) => {
  ...
  const handleMedicalCertChange = (e) => {
    ...
    if (file) {
      if (file.type !== "application/pdf" && !file.type.includes("image")) {
        setSubmitStatus({
          type: "error",
          message: "Please upload a PDF or image file for medical certificate.",
        });
        return;
      }
  ...
  const handleExperienceCertChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      if (file.type !== "application/pdf" && !file.type.includes("image")) {
        setSubmitStatus({
          type: "error",
          message:
            "Please upload a PDF or image file for experience certificate.",
        });
        return;
      }
      if (file.size > 5 * 1024 * 1024) {
  ...
  const handleSubmit = async (e) => {
    ...
    submitFormData.append("course_name", courseTitle);
    submitFormData.append("preferred_date", formData.preferred_date);
    submitFormData.append("experience_level", formData.experience_level);
    submitFormData.append("message", formData.message);

    if (medicalCert) {
      submitFormData.append("medical_certificate", medicalCert);
    }
    if (experienceCert) {
      submitFormData.append("experience_certificate", experienceCert);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4">
        ...
        {/* Modal Content */}
        <div className="relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 my-8 flex flex-col max-h-[90vh]">
          {/* Header - Fixed, no scroll */}
          <div className="bg-gradient-to-r from-teal-600 to-teal-600 text-white p-6 rounded-t-2xl flex-shrink-0">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-teal-500 transition-colors bg-gray-50">
                    {medicalCert ? (
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Experience Certificate (PDF/Image)
                  </label>
                  <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-teal-500 transition-colors bg-gray-50">
                    {experienceCert ? (
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <span className="text-[10px] text-gray-400">
                          {(experienceCert.size / 1024).toFixed(1)} KB
                        </span>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center gap-1">
                    ...
                    <input
                      type="file"
                      accept=".pdf,.jpg,.jpeg,.png"
                      onChange={handleExperienceCertChange}
                      className="hidden"
                    />
                  </label>
                </div>
...
export default ApplyModalWithCert;
